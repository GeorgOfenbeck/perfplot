...
class TestOverview extends Suite{
  ...
  def test_dgemm_seq() =
  {
    val sizes =  (for (i<-1 until 15) yield (i*100).toLong ).toList
  ...
  def test_daxpy_seq() =
  {
    val sizes_2power =  (for (i<-1 until 7) yield (Math.pow(10,i).toLong)).toList
  ...
  def test_dgmev_seq() =
  {
    val sizes_2power =  (for (i<-1 until 15) yield (i*100).toLong ).toList
