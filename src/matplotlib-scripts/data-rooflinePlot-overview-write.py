...
series = ['daxpy-cold', 'fft-MKL-cold', 'dgemv-cold', 'dgemm-cold'];
...
colors=[(0.2117, 0.467, 0.216), (0.258, 0.282, 0.725), (0.776,0.0196,0.07),(1,0,1) ,'#FFFF00' ]
fig = plt.figure()
...
for serie,i in zip(series,range(len(series))):
	...
	bytesTransferred = []
        file_in = open('bytes_write_'+serie+'.txt','r')
        lines = file_in.readlines()
        for line in lines:
                split_line = line.rstrip('\n').split(' ')
