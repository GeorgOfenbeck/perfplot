...
case class Counter(EventNum: String, Umask: String, name: String, Description: String, Comment: String)
{
  ...
  def CommenttoLong = Integer.parseInt(Comment,16).toLong
}
...
object JakeTown {
  ...
  Counter("07H","01H","LD_BLOCKS_PARTIAL.ADDRES S_ALIAS","False dependencies in MOB due to partial compare on address.",""),
  ...
  Counter("08H","01H","DTLB_LOAD_MISSES.MISS_CA USES_A_WALK","Misses in all TLB levels that cause a page walk of any page size.",""),
  Counter("08H","02H","DTLB_LOAD_MISSES.WALK_CO MPLETED","Misses in all TLB levels that caused page walk completed of any size.",""),
  Counter("08H","04H","DTLB_LOAD_MISSES.WALK_DU RATION","Cycle PMH is busy with a walk.",""),
  ...
  Counter("10H","20H","FP_COMP_OPS_EXE.SSE_FP_S CALAR_SINGLE","Counts number of SSE* single precision FP scalar uops executed.",""),
  Counter("10H","40H","FP_COMP_OPS_EXE.SSE_PACK ED SINGLE","Counts number of SSE* single precision FP packed uops executed.",""),
  ...
  Counter("24H","03H","L2_RQSTS.ALL_DEMAND_DAT A_RD","Counts any demand and L1 HW prefetch data load requests to L2.",""),
  ...
  Counter("2EH","4FH","LONGEST_LAT_CACHE.REFERE NCE","This event counts requests originating from the core that reference a cache line in the last level cache.","see Table 19-1"),
  ...
  Counter("49H","01H","DTLB_STORE_MISSES.MISS_CA USES_A_WALK","Miss in all TLB levels causes an page walk of any page size (4K/2M/4M/1G).",""),
  Counter("49H","02H","DTLB_STORE_MISSES.WALK_C OMPLETED","Miss in all TLB levels causes a page walk that completes of any page size (4K/2M/4M/1G).",""),
  Counter("49H","04H","DTLB_STORE_MISSES.WALK_D URATION","Cycles PMH is busy with this walk.",""),
  Counter("49H","10H","DTLB_STORE_MISSES.STLB_HI T","Store operations that miss the first TLB level but hit the second and do not cause page walks.",""),
  ...
  Counter("59H","80H","PARTIAL_RAT_STALLS.MUL_SI NGLE_UOP","Number of Multiply packed/scalar single precision uops allocated.",""),
  ...
  Counter("5BH","40H","RESOURCE_STALLS2.BOB_FUL L","Cycles Allocator is stalled due Branch Order Buffer.",""),
  Counter("5BH","4FH","RESOURCE_STALLS2.OOO_RS RC","Cycles stalled due to out of order resources full.",""),
  ...
  Counter("63H","02H","LOCK_CYCLES.CACHE_LOCK_D URATION","Cycles in which the L1D is locked.",""),
  ...
  Counter("85H","02H","ITLB_MISSES.WALK_COMPLET ED","Misses in all ITLB levels that cause completed page walks.",""),
  Counter("85H","04H","ITLB_MISSES.WALK_DURATIO N","Cycle PMH is busy with a walk.",""),
  ...
  Counter("88H","20H","BR_INST_EXEC.INDIRECT_NEA R_CALL","Qualify indirect near calls, including both register and memory indirect, executed.","Must combine with umask 80H"),
  ...
  Counter("88H","FFH","BR_INST_EXEC.ALL_BRANCHE S","Counts all near executed branches (not necessarily retired).",""),
  ...
  Counter("89H","08H","BR_MISP_EXEC.RETURN_NEA R","Qualify mispredicted indirect near branches that have a return mnemonic.","Must combine with umask 80H"),
  ...
  Counter("89H","20H","BR_MISP_EXEC.INDIRECT_NEA R_CALL","Qualify mispredicted indirect near calls, including both register and memory indirect, executed.","Must combine with umask 80H"),
  ...
  Counter("89H","FFH","BR_MISP_EXEC.ALL_BRANCHE S","Counts all near executed branches (not necessarily retired).",""),
  Counter("9CH","01H","IDQ_UOPS_NOT_DELIVERED.C ORE","Count number of non-delivered uops to RAT per thread.","Use Cmask to qualify uop b/w"),
  ...
  Counter("A1H","08H","UOPS_DISPATCHED_PORT.POR T_2_STA","Cycles which a store address uop is dispatched on port 2.",""),
  ...
  Counter("A3H","01H","CYCLE_ACTIVITY.CYCLES_L2_P ENDING","Cycles with pending L2 miss loads. Set AnyThread to count per core.",""),
  ...
  Counter("ABH","02H","DSB2MITE_SWITCHES.PENALT Y_CYCLES","Cycles DSB to MITE switches caused delay.",""),
  Counter("ACH","02H","DSB_FILL.OTHER_CANCEL","Cases of cancelling valid DSB fill not because of exceeding way limit.",""),
  Counter("ACH","08H","DSB_FILL.EXCEED_DSB_LINES","DSB Fill encountered > 3 DSB lines.",""),
  ...
  Counter("B0H","08H","OFFCORE_REQUESTS.ALL_DAT A_RD","Data read requests sent to uncore (demand and prefetch).",""),
  ...
  Counter("C1H","02H","OTHER_ASSISTS.ITLB_MISS_R ETIRED","Instructions that experienced an ITLB miss.",""),
  ...
  Counter("C3H","02H","MACHINE_CLEARS.MEMORY_O RDERING","Counts the number of machine clears due to memory order conflicts.",""),
  ...
  Counter("C4H","00H","BR_INST_RETIRED.ALL_BRAN CHES","Branch instructions at retirement.","See Table 19-1"),
  Counter("C4H","01H","BR_INST_RETIRED.CONDITION AL","Counts the number of conditional branch instructions retired.","Supports PEBS"),
  Counter("C4H","02H","BR_INST_RETIRED.NEAR_CALL","Direct and indirect near call instructions retired.",""),
  Counter("C4H","04H","BR_INST_RETIRED.ALL_BRAN CHES","Counts the number of branch instructions retired.",""),
  Counter("C4H","08H","BR_INST_RETIRED.NEAR_RET URN","Counts the number of near return instructions retired.",""),
  Counter("C4H","10H","BR_INST_RETIRED.NOT_TAKE N","Counts the number of not taken branch instructions retired.",""),
  Counter("C4H","20H","BR_INST_RETIRED.NEAR_TAK EN","Number of near taken branches retired.",""),
  Counter("C4H","40H","BR_INST_RETIRED.FAR_BRAN CH","Number of far branches retired.",""),
  Counter("C5H","00H","BR_MISP_RETIRED.ALL_BRAN CHES","Mispredicted branch instructions at retirement.","See Table 19-1"),
  Counter("C5H","01H","BR_MISP_RETIRED.CONDITION AL","Mispredicted conditional branch instructions retired.","Supports PEBS"),
  Counter("C5H","02H","BR_MISP_RETIRED.NEAR_CAL L","Direct and indirect mispredicted near call instructions retired.",""),
  Counter("C5H","04H","BR_MISP_RETIRED.ALL_BRAN CHES","Mispredicted macro branch instructions retired.",""),
  Counter("C5H","10H","BR_MISP_RETIRED.NOT_TAKE N","Mispredicted not taken branch instructions retired.",""),
  ...
  Counter("CAH","08H","FP_ASSIST.SIMD_OUTPUT","Number of SIMD FP assists due to output values.",""),
  Counter("CAH","10H","FP_ASSIST.SIMD_INPUT","Number of SIMD FP assists due to input values.",""),
  Counter("CAH","1EH","FP_ASSIST.ANY","Cycles with any input/output SSE* or FP assists.",""),
  Counter("CCH","20H","ROB_MISC_EVENTS.LBR_INSE RTS","Count cases of saving new LBR records by hardware.",""),
  ...
  Counter("CDH","02H","MEM_TRANS_RETIRED.PRECIS E_STORE","Sample stores and collect precise store operation via PEBS record. PMC3 only.","See Section 18.8.4.3"),
  ...
  Counter("D1H","04H","MEM_LOAD_UOPS_RETIRED.LL C_HIT","Retired load uops which data sources were data hits in LLC without snoops required.","Supports PEBS"),
  Counter("D1H","20H","MEM_LOAD_UOPS_RETIRED.LL C_MISS","Retired load uops which data sources were data missed LLC (excluding unknown data source).","Supports PEBS"),
  Counter("D1H","40H","MEM_LOAD_UOPS_RETIRED.HI T_LFB","Retired load uops which data sources were load uops missed L1 but hit FB due to preceding miss to the same cache line with data not ready.",""),
  Counter("D4H","02H","MEM_LOAD_UOPS_MISC_RETI RED.LLC_MISS","Retired load uops with unknown information as data source in cache serviced the load.","Supports PEBS. PMC0-3 only regardless HTT"),
  ...
  Counter("F2H","01H","L2_LINES_OUT.DEMAND_CLEA N","Clean L2 cache lines evicted by demand.",""),
  Counter("F2H","02H","L2_LINES_OUT.DEMAND_DIRT Y","Dirty L2 cache lines evicted by demand.",""),
